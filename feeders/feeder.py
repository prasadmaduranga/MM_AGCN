...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        try:
            with open(self.label_path, 'rb') as f:  # Open in binary mode
                self.sample_name, self.label = pickle.load(f)
        except UnicodeDecodeError:  # Specific to the utf-8 decode error
            print("UnicodeDecodeError: Trying with 'latin1' encoding for Python 2 compatibility.")
            with open(self.label_path, 'rb') as f:
                self.sample_name, self.label = pickle.load(f, encoding='latin1')
